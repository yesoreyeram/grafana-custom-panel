...
export const simplifyPanelData = (data: PanelData, hideFrameName = false): unknown => {
  const out: unknown[] = [];
  (data.series || []).forEach((s, index) => {
    const frameName = s.name || s.refId || `frame ${index}`;
    for (let i = 0; i < (s.length || 0); i++) {
      let a: any = {};
      if (!hideFrameName) {
        a.__frameName = frameName;
      }
      (s.fields || []).forEach((field) => {
        a[field.name] = field.values.get(i);
      });
      out.push(a);
    }
  });
